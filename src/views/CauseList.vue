<template>
	<section class="pt-4 lg:pt-12">
		<div class="pb-5 mb-2 md:mb-16 mx-auto w-11/12 py-6 lg:py-1">
                    ...
                    <div class="flex flex-col lg:grid grid-cols-3 gap-4 mb-4">
                        ...
                        <div class="h-full flex bg-white rounded-lg shadow-md">
                            <label for="search-dropdown" class="flex w-full items-center">
                                <select
                                    id="search-dropdown"
                                    class="h-12 w-full mx-2 focus:outline-none" placeholder="Filter by state"
                                    v-model="state"
                                    @change="filter('state')"
                                >
                                    <option value="">Filter by state</option>
                                    <option :value="state.name" v-for="(state, key) in states" :key="key">{{ state.name }}</option>
                                </select>
                            </label>
                        </div>
                        <div class="h-full flex bg-white rounded-lg shadow-md">
                            <label for="search-dropdown" class="flex w-full items-center">
                                ...
                                >
                                    ...
                                    <option :value="agency.shortname" v-for="(agency, key) in agencies" :key="key">{{ agency.name }}</option>
                                </select>
                            </label>
                        </div>
                    </div>
                    ...
                    <table class="table w-full">
                        ...
                        <tbody>
                            <tr class="text-md" v-for="(c, key) in temp_list" :key="key">
                                <td class="bg-hero lg:hidden p-4 text-md">{{ c.lead_defendant.name }} <span v-show="c.lead_defendant.designation">({{ c.lead_defendant.designation }})</span> {{ c.defendants_desc }}</td>
                                <td class="hidden lg:table-cell">{{ c.lead_defendant.name }} <span v-show="c.lead_defendant.designation">({{ c.lead_defendant.designation }})</span> {{ c.defendants_desc }}</td>
                                <td data-th="Alleged Offence">
                                    {{ c.amount }}
                                    <br v-if="c.amount">
                                    {{ c.type }}
                                </td>
                                <td data-th="Presiding Judge and Designated Court">{{ c.judge.name }}, {{ c.court.name }}, {{ c.court.state.name }}</td>
            ...
            <div class="h-64 flex items-center justify-center" v-if="temp_list.length < 1">
...
<script>
	...
	export default {
        ...
        methods: {
            getAll() {
                this.isLoading = true;
                axios.get(window.host + '/api/cause-list')
                .then(response => {
                    if (response.data) {
            ...
            search() {
                ...
                this.isLoading = true;
                axios.get(window.host + '/api/cause-list/search?q=' + this.q)
                .then(response => {
                    this.cause_list = [];
                    if (response.data)
                        this.cause_list = response.data;
            ...
            filter (field = '') {
                ...
                switch(field) {
                    ...
                    case 'agency':
                        ...
                        this.temp_list = this.cause_list.filter(function (item) {
                            return item.agency.shortname.toLowerCase().includes(q) || item.agency.name.toLowerCase().includes(q);
                        });
                        break;
                    case 'date':
                    ...
                    default:
                        ...
                        this.temp_list = this.cause_list.filter(function (item) {
                            return item.update.next_sitting.toLowerCase().includes(q) || item.agency.shortname.toLowerCase().includes(q) || item.agency.name.toLowerCase().includes(q) || item.court.state.name.toLowerCase().includes(q) || item.lead_defendant.name.toLowerCase().includes(q) || item.judge.name.toLowerCase().includes(q) || item.type.toLowerCase().includes(q);
                        });
            ...
            getAgencies() {
                axios.get(window.host + '/api/agencies')
                .then(response => {
                    if (response.data) {
