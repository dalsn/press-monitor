<template>
  <div class="">
	...
	<section class="flex flex-col mt-8 py-16">
		<div class="px-2 mx-auto md:px-10 xl:px-16 md:grid md:grid-cols-2 xl:grid-cols-3 lg:gap-6 gap-4">
			<CaseCard v-for="(c, key) in cases" :caseFile="c" :key="key" :className="{'active': index == key}"/>
		</div>
		<button class="btn mx-4 md:mx-auto mt-12 border border-transpurple xl:w-1/6 md:w-1/3 p-3 rounded-full hover:shadow-xl" @click="gotoCases">
			View All Cases
		</button>
	</section>
...
export default {
  ...
  methods: {
    ...
    getRecentCases() {
      axios.get(`${window.host}/api/recent-cases`)
      .then(response => {
        if (response.data) {
    ...
    startTimer() {
      this.timer = setTimeout(() => { this.changeCase('next') }, 20000);
    },
    stopTimer() {
